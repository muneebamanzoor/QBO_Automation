...
Accounts.forEach(({Salutation, FirstName, LastName, Title, SIN, DOB, Phone, Email, Street, PostalCode, City,Province, Country }) => {
  ...
  await page.getByLabel('Username').fill('cj.securityreview@cloudjunction.cloud');
  await page.getByLabel('Password').click();
  await page.getByLabel('Password').fill('Salesforce5');
  await page.getByRole('button', { name: 'Log In' }).click();
  await page.goto("https://cloudjunctionadvisors42-dev-ed.develop.lightning.force.com/lightning/o/Account/list?filterName=AllAccounts");
  ...
  // Click Edit, Clear and Change some fields and Save.
